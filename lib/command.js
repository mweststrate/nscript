		...
		return extend(self, {
			...
			wait: function() {
				...
				if (options.throwOnError) {
					if (status > 0)
						throw utils.buildError('NonZeroExitError', "Command '" + commandAndArgs.join(" ") + "' failed with status: " + status, status);
					else if (status < 0)
						throw utils.buildError('FailedToStartError', "Command '" + commandAndArgs.join(" ") + "' failed to start, got: '" + spawnError.message + "'. \nDoes '" + commandAndArgs[0] + "' exist (in the current dir or on PATH), is it readable and marked as executable?", 1);
				}
				return status;
			},
